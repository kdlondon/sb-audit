...
function ReportsContent(){
  ...
  const downloadPDF=async()=>{
    ...
    // MUST carry gw-shell: the chart blocks style themselves with the shell's custom
    // properties (ember, ink, paper). Rasterising the clone from document.body without it
    // would resolve them to nothing and print colourless charts.
    wrapper.className="gw-shell";
    wrapper.style.cssText="font-family:var(--font-body);color:var(--ink-900);background:var(--brand-white);padding:8px;";
    // PDF header
    const header=document.createElement("div");
    header.style.cssText="margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid var(--paper-edge);";
    header.innerHTML=`<h1 style="font-family:var(--font-display);font-size:24px;font-weight:700;color:var(--ink-900);margin:0 0 8px;">${title}</h1>
