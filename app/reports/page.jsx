...
- Return ONLY valid JSON: {"title":"...","slides":[...]}`;
  ...
  // Render from the v2 block model when the report has one: visual blocks become
  // components, consecutive text blocks are batched back into markdown so links,
  // citations and emphasis keep working exactly as before. Reports without blocks
  // (legacy, or generated before F0) fall through to the markdown path untouched.
  const renderDoc = (rep) => {
    const doc = rep?.content_blocks;
    if (!isV2(doc)) return renderContent(rep?.content || activeContent);
    const out = [];
    let buf = [];
    const flush = (k) => {
      if (!buf.length) return;
      out.push(<div key={`t${k}`}>{renderContent(buf.join("\n\n"))}</div>);
      buf = [];
    };
            ...
            <div className="bg-surface rounded-lg border border-main">
              ...
              <div className="flex">
                <div className="flex-1 px-8 py-6" ref={reportRef} data-report-content>
                  <div data-report-content>{viewingReport?renderDoc(viewingReport):renderContent(activeContent)}</div>
                  <Signature/>
                </div>
