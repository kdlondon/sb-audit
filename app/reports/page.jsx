      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="p-3">
          <div className="flex gap-1 flex-wrap mb-2">
            {e.competitor&&<Tag v={e.competitor}/>}
            {e.brand&&<span className="text-xs font-semibold text-main bg-surface2 px-1.5 py-0.5 rounded">{e.brand}</span>}
            {e.category&&<Tag v={e.category}/>}
            {e.year&&<span className="bg-surface2 px-1.5 py-0.5 rounded text-[11px] text-main">{e.year}</span>}
            {e.rating&&<span className="text-[11px]">{"★".repeat(Number(e.rating))}</span>}
          </div>
          {[["Type",e.type],["Portrait",e.portrait],["Phase",e.journey_phase],["Lifecycle",e.client_lifecycle],["Door",e.entry_door],["Role",e.bank_role],["Archetype",e.brand_archetype],["Tone",e.tone_of_voice],["Language",e.language_register],["Territory",e.primary_territory],["Execution",e.execution_style],["VP",e.main_vp],["Slogan",e.main_slogan]].filter(([,v])=>v&&v!==""&&!v.startsWith("Not ")&&!v.startsWith("None")).map(([l,v])=>(
...
function ReportsContent(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const[{data:local},{data:global}]=await Promise.all([supabase.from("audit_entries").select("*"),supabase.from("audit_global").select("*")]);
  ...
  // Search entries
  const searchResults=searchQuery.length>1?allData.filter(e=>{
    const q=searchQuery.toLowerCase();
    return(e.description||"").toLowerCase().includes(q)||(e.competitor||"").toLowerCase().includes(q)||(e.brand||"").toLowerCase().includes(q)||(e.main_slogan||"").toLowerCase().includes(q)||(e.synopsis||"").toLowerCase().includes(q);
  }).slice(0,10):[];

  const generate=async()=>{
    setGenerating(true);setReport("");
    const dataStr=filteredData.map(e=>`[${e.competitor||e.brand}] ${e.description||""} | Type:${e.type||""} | Portrait:${e.portrait||""} | Phase:${e.journey_phase||""} | Role:${e.bank_role||""} | Tone:${e.tone_of_voice||""} | Lang:${e.language_register||""} | Pain:${e.pain_point_type||""} | CTA:${e.cta||""} | Archetype:${e.brand_archetype||""} | Territory:${e.primary_territory||""} | Execution:${e.execution_style||""} | Insight:${(e.insight||"").slice(0,100)} | Synopsis:${(e.synopsis||"").slice(0,100)} | Transcript:${(e.transcript||"").slice(0,100)}`).join("\n");
    ...
    try{
      const response=await fetch("/api/ai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({use_opus:true,max_tokens:6000,
        system:`You are a world-class brand strategist ${scopeContext}. Write with authority and precision. Every claim must reference specific brands and data points. Structure with clear markdown: ## for main sections, ### for subsections, **bold** for key findings, bullet points for evidence. Be conclusive. Challenge assumptions where data supports it.`,
  ...
  return(
    <div className="min-h-screen flex" style={{background:"var(--bg)"}}>
      {/* Left: Report content */}
      <div className="flex-1 overflow-auto" style={{marginRight:viewerOpen?380:0,transition:"margin 0.15s"}}>
        <div className="bg-surface border-b border-main px-5 py-3 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-muted">Powered by Claude Opus</p>
          </div>
          {report&&(
            <button onClick={()=>setViewerOpen(!viewerOpen)} className={`px-3 py-1.5 text-xs rounded-lg font-medium border transition ${viewerOpen?"bg-accent-soft border-[var(--accent)] text-accent":"border-main text-muted hover:bg-surface2"}`}>
              {viewerOpen?"Hide":"Search entries"}
            </button>
          )}
        </div>
        ...
        <div className="p-5 max-w-4xl">
          ...
          <div className="bg-surface rounded-lg border border-main p-4 mb-3">
            ...
            <div className="flex gap-2">
              {[["local","Local"],["global","Global"],["combined","Combined"]].map(([k,l])=>(
          ...
          <div className="bg-surface rounded-lg border border-main p-4 mb-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {availableBrands.map(c=>(<button key={c} onClick={()=>toggleComp(c)} className={`px-3 py-1 rounded-full text-xs font-medium border transition ${competitors.includes(c)||competitors.length===0?"bg-accent-soft border-[var(--accent)] text-accent":"bg-surface border-main text-hint"}`}>{c}</button>))}
            </div>
            <p className="text-[10px] text-hint mt-1">{competitors.length===0?"All":""+competitors.length} — {filteredData.length} entries</p>
          </div>

          <div className="bg-surface rounded-lg border border-main p-4 mb-3">
            <h3 className="text-sm font-semibold text-main mb-2">Sections</h3>
            {currentSections.map(s=>(<label key={s.id} className="flex items-start gap-2 mb-2 cursor-pointer"><input type="checkbox" checked={sections.includes(s.id)} onChange={()=>toggleSec(s.id)} className="mt-0.5"/><div><div className="text-sm font-medium text-main">{s.label}</div><div className="text-xs text-hint">{s.desc}</div></div></label>))}
          </div>
          ...
          <button onClick={generate} disabled={generating||sections.length===0} className="bg-accent text-white px-6 py-2 rounded-lg text-sm font-semibold hover:opacity-90 disabled:opacity-50 mb-5">
      ...
      {/* Right: Floating entry viewer */}
      {viewerOpen&&(
        <div className="fixed top-0 right-0 w-[380px] h-screen bg-surface border-l border-main z-50 flex flex-col" style={{boxShadow:"-2px 0 12px rgba(0,0,0,0.05)"}}>
          {/* Search bar */}
          <div className="p-3 border-b border-main flex-shrink-0">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs font-semibold text-main">Search entries</span>
            ...
            <input value={searchQuery} onChange={e=>{setSearchQuery(e.target.value);setViewerEntry(null);}}
              placeholder="Search by brand, description, slogan..."
              className="w-full px-3 py-2 bg-surface2 border border-main rounded-lg text-sm text-main focus:outline-none focus:border-[var(--accent)]"/>
          </div>

          {/* Results or entry detail */}
          {viewerEntry?(
            <EntryViewer entry={viewerEntry} onClose={()=>setViewerEntry(null)}/>
          ):(
            <div className="flex-1 overflow-auto">
              {searchQuery.length<=1?(
                <div className="p-4 text-center text-hint text-sm">Type to search across all {allData.length} entries</div>
              ):searchResults.length===0?(
                <div className="p-4 text-center text-hint text-sm">No entries found</div>
              ):(
                <div className="p-2">
                  {searchResults.map(e=>(
                    <button key={e.id} onClick={()=>setViewerEntry(e)}
                      className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-accent-soft transition mb-1 border border-transparent hover:border-[var(--border)]">
                      <div className="flex gap-1.5 items-center mb-0.5">
                        {e.competitor&&<Tag v={e.competitor}/>}
                        {e.brand&&<span className="text-[10px] font-semibold text-main bg-surface2 px-1 rounded">{e.brand}</span>}
                        {e.year&&<span className="text-[10px] text-hint">{e.year}</span>}
                      </div>
                      <p className="text-xs font-medium text-main truncate">{e.description||"—"}</p>
                      {e.main_slogan&&<p className="text-[10px] text-muted truncate mt-0.5">{e.main_slogan}</p>}
                    </button>
                  ))}
                </div>
              )}
