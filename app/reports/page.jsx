...
const GLOBAL_SECTIONS = [
  ...
  { id: "execution", label: "Execution styles & patterns", desc: "How global brands execute their positioning — formats, styles, approaches" },
  { id: "archetypes", label: "Brand archetypes & roles", desc: "Which archetypes dominate and what roles brands cast themselves in" },
  { id: "insights", label: "Insights & ideas mapping", desc: "Human truths being activated and the creative concepts built on them" },
  { id: "inspiration", label: "Transferable inspiration", desc: "What Scotiabank could learn from the best global examples" },
];
...
function ReportsContent() {
  ...
  const filteredData = competitors.length > 0
    ? currentData.filter(e => competitors.includes(e.competitor || e.brand))
    : currentData;

  const availableBrands = scope === "local"
    ? OPTIONS.competitor.filter(c => c !== "Other")
  ...
  const generate = async () => {
    ...
    try {
      const response = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          max_tokens: 4000,
          system: `You are a senior brand strategist ${scopeContext}. Be conclusive and strategic. Lead with insights, support with evidence. Use brand strategy language. Be specific about which brands do what.`,
          messages: [{ role: "user", content: `Here is audit data for ${filteredData.length} pieces:\n${dataStr}\n\nGenerate a strategic report covering: ${sectionNames}\n\n${customInstructions ? `Additional instructions: ${customInstructions}` : ""}\n\nFormat in Markdown.` }],
        }),
      });
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      <div className="bg-surface border-b border-main px-5 py-3">
        ...
        <p className="text-xs text-muted">Generate strategic analysis from your audit data</p>
      </div>
      ...
      <div className="p-5 max-w-4xl">
          ...
          <div className="flex gap-2">
            {[["local","Local (Canadian market)"],["global","Global (creative benchmarks)"],["combined","Combined"]].map(([k,l]) => (
              <button key={k} onClick={() => setScope(k)}
                className={`px-4 py-1.5 rounded-lg text-xs font-medium border transition ${scope === k ? "bg-accent-soft border-[var(--accent)] text-accent" : "bg-surface border-main text-muted"}`}>{l}</button>
            ))}
          </div>
        </div>
            ...
            {availableBrands.map(c => (
              <button key={c} onClick={() => toggleComp(c)}
                className={`px-3 py-1 rounded-full text-xs font-medium border transition ${
                  competitors.includes(c) || competitors.length === 0 ? "bg-accent-soft border-[var(--accent)] text-accent" : "bg-surface border-main text-hint"
                }`}>{c}</button>
            ))}
          </div>
        ...
        <div className="bg-surface rounded-lg border border-main p-4 mb-3">
          ...
          {currentSections.map(s => (
            <label key={s.id} className="flex items-start gap-2 mb-2 cursor-pointer">
              ...
              <div>
                <div className="text-sm font-medium text-main">{s.label}</div>
                <div className="text-xs text-hint">{s.desc}</div>
              </div>
            </label>
          ))}
        </div>
        ...
        {report && (
          <div className="bg-surface rounded-lg border border-main p-5">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-semibold text-main">Generated report</h3>
              <div className="flex gap-2">
                <button onClick={copyReport} className="px-3 py-1 text-xs border border-main rounded-lg text-muted hover:bg-surface2">
                  {copied ? "Copied!" : "Copy"}
                </button>
                <button onClick={downloadMD} className="px-3 py-1 text-xs border border-main rounded-lg text-muted hover:bg-surface2">
                  Download .md
                </button>
              </div>
            </div>
