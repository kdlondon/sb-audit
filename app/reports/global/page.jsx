...
const SECTION_LIST = [
  { key: "rationale", label: "Curation rationale", desc: "What the set covers and the logic behind the selection" },
  { key: "cases", label: "The cases", desc: "Curated gallery — what / why it works / transferable idea" },
  { key: "patterns", label: "Patterns", desc: "What recurs across the best cases" },
  { key: "plays", label: "Transferable plays", desc: "Concrete plays the client could run, ICP-lensed" },
];
...
function GlobalInner() {
  ...
  const [brand, setBrand] = useState("");        // optional single-source focus ("" = all)
  ...
  const [filters, setFilters] = useState({ brands: [], territories: [], archetypes: [], yearFrom: "", yearTo: "", minRating: 4 });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const fwNames = (framework?.globalBenchmarks || []).map((g) => g?.name).filter(Boolean);
        const supabase = createClient();
        const { data } = await supabase.from("creative_source").select("competitor,brand,brand_name,primary_territory,brand_archetype,year,rating").eq("project_id", projectId).eq("scope", "global");
        const rws = (data || []).map((r) => ({ brand: r.competitor || r.brand || r.brand_name || "—", territory: r.primary_territory || "", archetype: r.brand_archetype || "", year: r.year, rating: Number(r.rating) || 0 }));
        setRows(rws);
        let list = fwNames;
        if (!list.length) list = [...new Set(rws.map((r) => r.brand))].filter((b) => b && b !== "—");
        setBrands(list);
        setTerritories([...new Set(rws.flatMap((r) => r.territory.split(",").map((x) => x.trim()).filter(Boolean)))].sort());
        setArchetypes([...new Set(rws.map((r) => r.archetype).filter(Boolean))].sort());
  ...
  const inRange = useMemo(() => rows.filter((r) => {
    ...
    if (filters.territories.length && !r.territory.split(",").map((x) => x.trim()).some((t) => filters.territories.includes(t))) return false;
    if (filters.archetypes.length && !filters.archetypes.includes(r.archetype)) return false;
    ...
    return true;
  }).length, [rows, filters, brand]);

  const bodyFor = (extra) => ({
    project_id: projectId, brand, icp,
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch("/api/reports/global", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bodyFor()) });
  ...
  const regenerate = async (key) => {
    ...
    try {
      const res = await fetch("/api/reports/global", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bodyFor({ section: key, priorSections: report?.sections || [] })) });
      const dt = await res.json();
  ...
  const ensureSaved = async () => {
    const supabase = createClient();
    const subject = report.meta?.subject || "Global";
    const title = `Global Creative Inspiration — ${projectName} · ${subject}`;
    ...
    const { error } = await supabase.from("saved_reports").insert({ id, title, content, template_type: "global_inspiration", scope: "global", project_id: projectId });
    if (error) throw new Error(error.message);
    setSavedId(id);
    return id;
  };
  ...
  const openCite = async (rawId) => {
    ...
    try {
      const supabase = createClient();
      const { data } = await supabase.from("creative_source").select("id,competitor,brand,brand_name,description,synopsis,image_url,url,year,rating,primary_territory,communication_intent").eq("id", id).maybeSingle();
  ...
  const filtersActive = filters.brands.length || filters.territories.length || filters.archetypes.length || filters.yearFrom || filters.yearTo || filters.minRating !== 4;

  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      <style>{`@media print { .no-print { display: none !important; } body * { visibility: hidden; } #global-report, #global-report * { visibility: visible; } #global-report { position: absolute; left: 0; top: 0; width: 100%; border: none; padding: 0; } }`}</style>
      <div className="no-print"><Nav /></div>
      <div className="max-w-[860px] mx-auto px-6 pb-24" style={{ paddingTop: "calc(var(--sec-h) + 20px)" }}>
        <div className="no-print">
          <h1 className="text-2xl font-bold text-main">Global Creative Inspiration</h1>
          <p className="text-sm text-muted mt-1">The best and most distinctive global cases as inspiration — curated by rating. Configure once, then refine, regenerate or edit any section.</p>
            ...
            <div className="mb-6 space-y-3">
              <div className="bg-surface rounded-lg border border-main p-4">
                <div className="flex flex-wrap items-end gap-x-6 gap-y-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-main mb-2">Minimum rating</h3>
                    <Toggle value={String(filters.minRating)} set={(v) => setFilterField("minRating", Number(v))} options={[["0", "Any"], ["3", "3★+"], ["4", "4★+"], ["5", "5★"]]} />
                    <p className="text-[10px] text-hint mt-1.5">Curation floor — only standout cases above this rating</p>
                  </div>
                  <div className="ml-auto mb-1 text-xs text-hint">{inRange} of {rows.length} cases in range</div>
                </div>
              </div>

              <div className="bg-surface rounded-lg border border-main p-4">
                <h3 className="text-sm font-semibold text-main mb-2">Global brands</h3>
              ...
              {territories.length > 0 && (
                <div className="bg-surface rounded-lg border border-main p-4">
                  <h3 className="text-sm font-semibold text-main mb-2">Territories</h3>
                  <div className="flex gap-2 flex-wrap">{territories.map((t) => <Chip key={t} on={filters.territories.includes(t)} onClick={() => toggleFilter("territories", t)}>{t}</Chip>)}</div>
                  <p className="text-[10px] text-hint mt-2">{filters.territories.length === 0 ? "All territories included" : `${filters.territories.length} selected`}</p>
                </div>
              )}

              {archetypes.length > 0 && (
                <div className="bg-surface rounded-lg border border-main p-4">
                  <h3 className="text-sm font-semibold text-main mb-2">Archetypes</h3>
                  <div className="flex gap-2 flex-wrap">{archetypes.map((a) => <Chip key={a} on={filters.archetypes.includes(a)} onClick={() => toggleFilter("archetypes", a)}>{a}</Chip>)}</div>
                  <p className="text-[10px] text-hint mt-2">{filters.archetypes.length === 0 ? "All archetypes included" : `${filters.archetypes.length} selected`}</p>
                </div>
              )}
          ...
          {loading && <p className="text-sm text-accent animate-pulse">Curating the strongest cases and composing the report… this takes about a minute.</p>}
        </div>

        {report && (
          <div id="global-report" className="bg-surface border border-main rounded-2xl px-8 py-10">
            <div className="text-[10px] font-mono uppercase tracking-[0.18em] text-hint">Global Creative Inspiration · {projectName} · {report.meta?.subject || "All global"} · {report.meta?.icp} lens</div>
            ...
            <div className="text-[10px] text-hint mt-8 font-mono">{report.meta?.cases} cases · {report.meta?.rated} rated 4★+ · {report.meta?.brands} brands · {report.meta?.yearRange}</div>
          </div>
        )}
      </div>
          ...
          <div className="relative bg-surface border border-main rounded-2xl shadow-2xl w-full max-w-[480px] max-h-[85vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5">
              ...
              <div className="flex gap-2 flex-wrap mb-2">
                ...
                {cite.rating && <span className="text-[10px] text-amber-500 bg-surface2 px-1.5 py-0.5 rounded">{"★".repeat(Number(cite.rating))}</span>}
                {cite.primary_territory && <span className="text-[10px] text-muted bg-surface2 px-1.5 py-0.5 rounded">{cite.primary_territory}</span>}
              </div>
