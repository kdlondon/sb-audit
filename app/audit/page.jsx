...
function AuditContent({scope,onScopeChange,pendingForm,clearPendingForm}){
  ...
  const fetchYTMeta=async(url)=>{
    ...
    try{
      ...
      if(!meta.error){
        ...
        if(meta.description)updates.synopsis=meta.description;
  ...
  const analyzeWithAI=async()=>{
    ...
    const notes=cur.analyst_comment;
    if(!imgUrl&&!transcript)return;
    setAnalyzing(true);
    try{
      let context=[];
  ...
  const openForm=(entry)=>{
    ...
    if(ytId(e.url))setMaterialType("video");else if(e.image_url)setMaterialType("image");else if(e.url)setMaterialType("web");else setMaterialType("none");
  ...
  if(vw==="form"){
    ...
    const hasContent=y||(imgUrl&&isImgUrl(imgUrl))||cur.url||cur.transcript||cur.analyst_comment;
    ...
    return(
      <div className="min-h-screen" style={{background:"var(--bg)"}}>
        ...
        <div className="flex" style={{height:"calc(100vh - 52px)"}}>
          {/* Left panel: material + transcript + notes */}
          <div className="flex-1 flex flex-col overflow-hidden">
            {/* Material type + URL input */}
            <div className="bg-surface border-b border-main px-4 py-3 flex-shrink-0">
              {materialType==="none"?(
                <div>
                  ...
                  <div className="flex gap-2">
                    {[["video","Video URL"],["web","Website URL"],["image","Image"]].map(([k,l])=>(
                      <button key={k} onClick={()=>setMaterialType(k)} className="flex-1 py-3 rounded-lg border border-main text-sm font-medium text-main hover:bg-accent-soft hover:border-[var(--accent)] transition text-center">{l}</button>
                    ))}
                  </div>
                </div>
              ):(
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <div className="flex bg-surface2 rounded-lg p-0.5">
                      {[["video","Video"],["web","Website"],["image","Image"]].map(([k,l])=>(
                        <button key={k} onClick={()=>{setMaterialType(k);if(k!=="image")setCur(prev=>({...prev,url:"",image_url:prev.image_url||""}));if(k==="image")setCur(prev=>({...prev,url:""}));}}
                  ...
                  {materialType==="image"&&(
                    <div className="flex gap-2"><div className="flex-1"><label className="block text-[9px] text-hint uppercase font-semibold mb-0.5">Image URL</label><input value={cur.image_url||""} onChange={e=>setImageFromUrl(e.target.value)} placeholder="https://...image.jpg" className="w-full px-2 py-1.5 bg-surface2 border border-main rounded text-sm text-main" /></div>
                    <div className="flex items-end"><label className="px-3 py-1.5 bg-surface2 border border-main rounded text-xs text-muted cursor-pointer hover:bg-accent-soft">{uploading?"...":"Upload"}<input type="file" accept="image/*" onChange={e=>uploadImage(e.target.files?.[0])} className="hidden" /></label></div></div>
                  )}
                </div>
              )}
            </div>

            {/* Scrollable: preview + transcript + notes */}
            <div className="flex-1 overflow-auto" onDrop={handleDrop} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}>
              {/* Preview area */}
              <div className={`flex items-center justify-center p-4 min-h-[250px] transition ${dragOver?"ring-2 ring-[var(--accent)] ring-inset":""}`}
                style={{background:dragOver?"var(--accent-soft)":"var(--surface2)"}}>
                {materialType==="video"&&y?(
                  <iframe width="100%" height="350" style={{maxWidth:700}} src={`https://www.youtube.com/embed/${y}`} frameBorder="0" allowFullScreen className="rounded-lg" />
                ):materialType==="image"&&imgUrl&&isImgUrl(imgUrl)?(
                  <img src={imgUrl} className="max-w-full max-h-[400px] rounded-lg" alt="" />
                ):materialType==="web"&&cur.url?(
                  <div className="w-full flex flex-col" style={{height:350}}>
                    <iframe src={cur.url} width="100%" className="rounded-lg border border-main flex-1" sandbox="allow-scripts allow-same-origin" />
                    <div className="mt-2 text-center"><a href={cur.url} target="_blank" rel="noopener" className="text-xs text-accent hover:underline">Open in new tab ↗</a><p className="text-[10px] text-hint mt-0.5">Some sites block embedding</p></div>
                  </div>
                ):(
              ...
              {/* Transcript + Analyst notes — always visible below preview */}
              <div className="bg-surface border-t border-main px-4 py-3 space-y-3">
                <div style={fieldStyle("transcript")}>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-[10px] text-muted uppercase font-semibold">Transcript / Copy</label>
                    <span className="text-[9px] text-hint">Paste from YouTube or type what you see</span>
                  </div>
                  <textarea value={cur.transcript||""} onChange={e=>setCur({...cur,transcript:e.target.value})}
                    rows={4} placeholder="Paste the video transcript, ad copy, or any text content here..."
                    className="w-full px-3 py-2 bg-surface2 border border-main rounded-lg text-sm text-main resize-y" />
                </div>
                <div style={fieldStyle("analyst_comment")}>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-[10px] text-muted uppercase font-semibold">Analyst notes</label>
                    <span className="text-[9px] text-hint">Your observations — also sent to AI for better analysis</span>
                  </div>
                  <textarea value={cur.analyst_comment||""} onChange={e=>setCur({...cur,analyst_comment:e.target.value})}
                    rows={3} placeholder="What stands out? Initial observations, strategic notes, context..."
                    className="w-full px-3 py-2 bg-surface2 border border-main rounded-lg text-sm text-main resize-y" />
                </div>

                {/* Analyze button */}
                {(cur.image_url||cur.transcript||cur.analyst_comment)&&(
                  <button onClick={analyzeWithAI} disabled={analyzing}
                    className="text-sm bg-accent-soft text-accent border border-[var(--accent)] px-4 py-2 rounded-lg font-medium hover:opacity-80 disabled:opacity-50 w-full">
          ...
          <div className="w-[380px] border-l border-main bg-surface overflow-auto">
            ...
            <div className="p-3 border-t border-main">
              <button onClick={save} className="w-full bg-accent text-white py-2 rounded-lg text-sm font-semibold hover:opacity-90">{eid?"Save changes":"Save entry"}</button>
            </div>
          </div>
        </div>
      </div>
