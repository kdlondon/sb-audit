...
Be analytical and conclusive, not merely descriptive. Find patterns, contrasts, and strategic implications across the cases.`;
  ...
  const fetchIGMeta=async(url)=>{
    setYtLoading(true);
    try{const res=await fetch("/api/instagram",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url})});const meta=await res.json();
      if(!meta.error){const updates={type:"Social post"};if(meta.caption){updates.description=meta.caption.split("\n")[0].slice(0,120);updates.synopsis=meta.caption;}if(meta.thumbnail)updates.image_url=meta.thumbnail;if(meta.transcript)updates.transcript=meta.transcript;if(meta.year)updates.year=meta.year;if(meta.owner){if(scope==="global")updates.brand=meta.owner;else updates.competitor=meta.owner;}autoFill(updates);}
      else{setToast({message:"Instagram: "+meta.error});}
    }catch{}setYtLoading(false);
  };
  const setSocialUrl=(url)=>{setCur(prev=>({...prev,url}));setMaterialType("social");if(/instagram\.com/i.test(url))fetchIGMeta(url);};
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16,minWidth:0}}>
            <div>
              {materialType==="none"?(<div><p className="mb-2.5 gw-flabel text-[10px] uppercase">Choose material type</p><div className="flex gap-2 flex-wrap">{[["video","Video URL"],["videoFile","Video File"],["web","Website URL"],["social","Social"],["image","Image"],["document","Document"]].map(([k,l])=>(<button key={k} onClick={()=>setMaterialType(k)} className="gw-tbtn flex-1 min-w-[100px] text-center" style={{padding:"12px 10px",borderRadius:9,border:"1px solid var(--border-hairline)",background:"var(--brand-white)",fontFamily:"var(--font-mono)",fontSize:11.5,color:"var(--ink-800)",cursor:"pointer"}}>{l}</button>))}</div></div>
                ...
                {materialType==="social"&&<div style={{display:"flex",flexDirection:"column",gap:16}}>
                  <div style={{display:"flex",gap:6}}>
                    {[["single","Single post"],["feed","Profile feed"]].map(([k,l])=>(
                      <button key={k} onClick={()=>setSocialMode(k)} className={socialMode===k?"gw-chp on":"gw-chp"} style={{borderRadius:8}}>{l}</button>
                    ))}
                  </div>
                  {socialMode==="single"
            ...
            <div onDrop={handleDrop} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}>
              <div className={`flex items-center justify-center transition ${dragOver?"ring-2 ring-[var(--accent-ember)] ring-inset":""}`} style={{border:"1px solid var(--border-hairline)",borderRadius:14,background:dragOver?"var(--accent-soft)":"var(--brand-white)",padding:18,minHeight:250}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:16}}>
                <div style={fieldStyle("transcript")}><div className="flex justify-between items-baseline" style={{marginBottom:6}}><label className="gw-flabel" style={{marginBottom:0}}>Transcript / Copy</label><span className="gw-fhint">{ytLoading&&!cur.transcript?<span className="text-accent animate-pulse">Bringing the transcript…</span>:"Paste from YouTube or type what you see"}</span></div><textarea value={cur.transcript||""} onChange={e=>setCur({...cur,transcript:e.target.value})} rows={4} placeholder={ytLoading&&!cur.transcript?"Bringing the transcript…":"Paste the video transcript, ad copy, or any text content here..."} className="w-full gw-finput gw-fta" /></div>
                <div style={fieldStyle("analyst_comment")}><div className="flex justify-between items-baseline" style={{marginBottom:6}}><label className="gw-flabel" style={{marginBottom:0}}>Analyst notes</label><span className="gw-fhint">Your observations — also sent to AI</span></div><textarea value={cur.analyst_comment||""} onChange={e=>setCur({...cur,analyst_comment:e.target.value})} rows={3} placeholder="What stands out? Initial observations, strategic notes..." className="w-full gw-finput gw-fta" /></div>
                {cur.type==="Social post"&&<div><div className="flex justify-between items-baseline" style={{marginBottom:6}}><label className="gw-flabel" style={{marginBottom:0}}>Caption</label><span className="gw-fhint">Post text — sent to the AI</span></div><textarea value={cur.custom_dimensions?._meta?.caption||""} onChange={e=>setCur(prev=>({...prev,custom_dimensions:{...(prev.custom_dimensions||{}),_meta:{...((prev.custom_dimensions||{})._meta||{}),caption:e.target.value}}}))} rows={4} placeholder="Social post caption / copy..." className="w-full gw-finput gw-fta" /></div>}
                          ...
                          if (f.type === "textarea") return (
                            <div key={f.key} style={fieldStyle(dbKey)} className="rounded px-1 -mx-1">
                              ...
                              <textarea value={val} onChange={e => setVal(e.target.value)} rows={2}
                                className="w-full gw-finput gw-fta" />
                            </div>
                          );
  ...
  return(
    <div className="min-h-screen" style={{background:viewMode==="collections"&&activeCollection?"#ebebeb":"transparent",backgroundImage:viewMode==="collections"&&activeCollection?"radial-gradient(circle, #c8c8c8 1px, transparent 1px)":"none",backgroundSize:"22px 22px"}}>
      ...
      {sb&&!sb._mapCountry&&(
        <QuickLook entry={sb} onClose={()=>setSb(null)} onEdit={openForm} onFull={openFull}
          onPdf={downloadCase} downloading={downloading} onZoomImage={setZoomImg}/>
      )}
