...
function ShowcasePage() {
  ...
  // Handle shared showcase link OR direct view by ID (from report editor)
  useEffect(() => {
    const viewId = searchParams.get("view");
    if (!viewId || typeof window === "undefined") return;

    // First check sessionStorage (from /showcase/[id] redirect)
    ...
    // Otherwise load directly from DB (from report editor "Generate Showcase")
    (async () => {
      const { data } = await supabase.from("saved_showcases").select("*").eq("id", viewId).single();
      if (data) {
        setCurrentShowcase(data);
        setCurrentSlide(0);
        setView("present");
      }
    })();
    ...
    <AuthGuard><ProjectGuard><Nav />
      <div className="min-h-screen" style={{ background: "var(--bg)" }}>
        ...
        <div className="section-bar px-5 py-3 flex justify-between items-center">
          ...
          {canEdit && (
            <button onClick={() => setView("create")} className="px-3 py-1.5 text-sm text-white rounded-lg font-semibold hover:opacity-90"
