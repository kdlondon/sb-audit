...
const SLIDE_COLORS = [
  ...
  { id: "navy", label: "Navy", color: "#0a0f3c" },
  { id: "charcoal", label: "Charcoal", color: "#1e1a22" },
  { id: "black", label: "Black", color: "#111015" },
  { id: "white", label: "White", color: "#ffffff" },
  { id: "cream", label: "Cream", color: "#faf5ee" },
  { id: "lavender", label: "Lavender", color: "#e8e0f0" },
  { id: "mint", label: "Mint", color: "#e0f5ee" },
  { id: "blush", label: "Blush", color: "#fce4ec" },
  { id: "sky", label: "Sky", color: "#e3f2fd" },
  { id: "chartreuse", label: "Chartreuse", color: "#D4E520" },
  { id: "electric", label: "Electric", color: "#0019FF" },
];
            ...
            <div className="space-y-1">
              {editSlides.map((slide, idx) => (<>
                ...
                <div key={idx} className="bg-surface border border-main rounded-xl overflow-hidden">
                  <div className="flex justify-between items-center px-4 py-2.5 cursor-pointer hover:bg-surface2 transition"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {slide.title && <span className="text-[10px] text-muted truncate max-w-[200px]">— {slide.title}</span>}
                    </div>
                    <div className="flex items-center gap-1" onClick={e=>e.stopPropagation()}>
                      {/* Color picker */}
                      <div className="flex items-center gap-0.5 mr-2">
                        {SLIDE_COLORS.slice(0,8).map(sc=>(
                      ...
                      {/* Preview */}
                      <button onClick={()=>setPreviewSlide(previewSlide===idx?null:idx)}
                        className={`text-[9px] px-2 py-1 rounded transition ${previewSlide===idx?"bg-accent text-white":"text-muted hover:text-main hover:bg-surface2"}`}>Preview</button>
                  ...
                  {previewSlide===idx&&(
                    <div className="border-t border-main">
                      <div className="aspect-video relative overflow-hidden" style={{backgroundColor:getThemeForSlide(slide,idx).bg,maxHeight:320}}>
                        <div className="absolute inset-0 flex items-center justify-center p-8" style={{transform:"scale(0.55)",transformOrigin:"center center"}}>
                  ...
                  {!collapsedSlides.has(idx)&&<div className="px-5 pb-5 pt-3 border-t border-main space-y-3">
                    {/* Slide type selector */}
                    <div>
                        ...
                        {(slide.entries||[]).length > 0 && (
                          <div className="space-y-1.5 mb-3">
                            {(slide.entries||[]).map((entry, ei) => {
                              ...
                              return (
                                <div key={ei} className="flex items-center gap-3 bg-surface2 rounded-lg p-2 group">
                                  {thumb && <img src={thumb} className="w-14 h-10 object-cover rounded flex-shrink-0" alt="" />}
                                  ...
                                  <button onClick={() => {
                                    ...
                                    s[idx] = { ...s[idx], entries };
                                    setEditSlides(s);
                                  }} className="text-red-400 hover:text-red-600 text-sm px-2 py-1 rounded hover:bg-red-50 opacity-0 group-hover:opacity-100 transition flex-shrink-0" title="Remove">×</button>
                                </div>
                              );
                            })}
                          </div>
