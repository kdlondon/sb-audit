...
Fields: title ("Thank You"), subtitle ("Generated by Knots & Dots — Category Landscape Platform"), date ("${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}")`;
  ...
  const enterEdit = () => { setEditSlides(JSON.parse(JSON.stringify(currentShowcase.slides))); setEditTitle(currentShowcase.title); nav({ id: currentShowcase.id, edit: 1 }); };
              ...
              {editSlides.map((slide, idx) => (
                <div key={idx} className="bg-surface border border-main rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Entries editor — for CS slides with entries arrays */}
                    {Array.isArray(slide.entries) && slide.entries.length > 0 && (
                      <div>
                        <label className="block text-[10px] text-muted uppercase font-semibold mb-2">
                          Entries ({slide.entries.length})
                          <span className="text-hint font-normal ml-1">— click × to remove</span>
                        </label>
                          ...
                          {slide.entries.map((entry, ei) => {
                            ...
                            return (
                              <div key={ei} className="flex items-center gap-3 bg-surface2 rounded-lg p-2 group">
                                {thumb && <img src={thumb} className="w-16 h-12 object-cover rounded flex-shrink-0" alt="" />}
                                ...
                                <button onClick={() => {
                                  ...
                                  s[idx] = { ...s[idx], entries };
                                  setEditSlides(s);
                                }} className="text-red-400 hover:text-red-600 text-sm px-2 py-1 rounded hover:bg-red-50 opacity-0 group-hover:opacity-100 transition flex-shrink-0" title="Remove entry">×</button>
                              </div>
                            );
                          })}
                        </div>
