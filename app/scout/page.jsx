...
const REGION_CODES = [
  ...
  { code: "PT", label: "Portugal" }, { code: "NL", label: "Netherlands" },
  ...
  { code: "DK", label: "Denmark" }, { code: "NO", label: "Norway" },
  { code: "FI", label: "Finland" }, { code: "PL", label: "Poland" },
  { code: "CZ", label: "Czech Republic" }, { code: "GR", label: "Greece" },
  ...
  { code: "CL", label: "Chile" }, { code: "PE", label: "Peru" },
  { code: "EC", label: "Ecuador" }, { code: "UY", label: "Uruguay" },
  { code: "CR", label: "Costa Rica" }, { code: "PA", label: "Panama" },
  ...
  { code: "PH", label: "Philippines" }, { code: "TW", label: "Taiwan" },
  { code: "HK", label: "Hong Kong" }, { code: "VN", label: "Vietnam" },
  { code: "AE", label: "UAE" }, { code: "SA", label: "Saudi Arabia" },
  { code: "IL", label: "Israel" }, { code: "EG", label: "Egypt" },
...
export default function ScoutPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // The PRINCIPAL brand (study subject) belongs in the list too.
      const principal = framework?.principalBrand?.name || framework?.brandName;
      if (principal) brands.add(principal);
...
- Return ONLY valid JSON, no markdown`,
  ...
  const handleImportSaved = async (item) => {
    ...
    // Stills are captured in the Creative Source editor after import, so the entry
    // starts from the source thumbnail.
    ...
    <AuthGuard><ProjectGuard>
      ...
      <main style={{ flex: 1, minWidth: 0, height: "100vh", display: "flex", flexDirection: "column", position: "relative" }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "24px 34px 56px" }}>
          ...
          {showResults && (
            <div>
              {/* Results / Saved — a content control, not N2: same white segmented
                  pill as the other in-view filters, ink-800 active. */}
              <div style={{ display: "inline-flex", gap: 2, background: "var(--brand-white)", border: "1px solid var(--border-hairline)", borderRadius: 10, padding: 3, marginBottom: 18 }}>
                {[
                  { id: false, label: `Results (${filteredVideos.length})` },
                  { id: true, label: `Saved (${savedItems.length})`, mark: true },
                ].map(t => {
                  const on = savedTab === t.id;
                  return (
                    <button key={String(t.id)} onClick={() => setSavedTab(t.id)}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 13px", borderRadius: 8, border: "none", cursor: "pointer", background: on ? "var(--ink-800)" : "transparent", color: on ? "var(--brand-cream)" : "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: on ? 600 : 500 }}>
                      {t.mark && <svg width="11" height="11" viewBox="0 0 24 24" fill={on ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z" /></svg>}
                      {t.label}
                    </button>
                  );
                })}
