...
export function socialVisuals(key, S = {}) {
  const { perBrand = [], pillarLandscape = [], formatMix = [], platformMix = [], totalPosts = 0, brandCount = 0, windowLabel = "", compare = null } = S;
  const byEng = [...perBrand].sort((a, b) => num(b.avgEng) - num(a.avgEng));
  const leader = byEng[0];

  switch (key) {
    case "declared_deployed":
      return compareTable(compare, key, { title: "Declared vs deployed" });
    case "snapshot": {
      const out = [];
      out.push(kpiRow([
...
export function flagshipVisuals(key, S = {}) {
  const { territories = [], brandCount = 0, totalPieces = 0, inRange = 0, gaps = null, compare = null, plays = null } = S;
  switch (key) {
    case "landscape": {
    ...
    case "positioning":
      return compareTable(compare, key, { title: "Declared vs demonstrated" });
    case "recommendations":
      return playsList(plays, key, "What to do");
    case "exec": {
      return [kpiRow([
...
// The declares / demonstrates / gap table. Both Positioning x-ray and Declared vs deployed
// are the same comparison, so one builder serves both. It is ANALYST READ by nature: a gap
// between what a brand says and what it shows is a judgement, never a count.
export function compareTable(rows, sectionKey, labels = {}) {
  const list = (Array.isArray(rows) ? rows : []).filter((r) => r && r.brand);
  if (list.length < 2) return [];
  return [makeBlock("compare", { sectionKey, data: {
    items: list.slice(0, 7).map((r) => ({
      brand: String(r.brand).slice(0, 40),
      declares: String(r.declares || "").trim(),
      demonstrates: String(r.demonstrates || "").trim(),
...
// Prioritised actions. Impact/effort only travel when the engine returned them.
export function playsList(items, sectionKey, title) {
  const list = (Array.isArray(items) ? items : []).filter((p) => p && (p.name || p.move));
  if (!list.length) return [];
  return [makeBlock("plays", { sectionKey, data: {
    title,
    items: list.slice(0, 8).map((p) => ({
      name: String(p.name || "").trim(),
      move: String(p.move || "").trim(),
      impact: p.impact ? String(p.impact).trim() : undefined,
      effort: p.effort ? String(p.effort).trim() : undefined,
    })),
  } })];
}
...
export function globalVisuals(key, S = {}) {
  ...
  switch (key) {
    ...
    case "plays":
      return playsList(plays, key, "Transferable plays");
    default: return [];
  }
}
...
export function innovationVisuals(key, S = {}) {
  ...
  switch (key) {
    ...
    case "recommendations":
      return playsList(plays, key, "Where to play");
    default: return [];
  }
}
