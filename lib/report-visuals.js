...
export const quadrant = ({ points, xLabel, yLabel, title }, sectionKey) =>
  makeBlock("quadrant", { sectionKey, data: { points, xLabel, yLabel, title } });
...
export function flagshipVisuals(key, S = {}) {
  ...
  switch (key) {
    case "landscape": {
      ...
      return [rankingBars(territories.slice(0, 8).map((t) => ({ label: t.name, value: t.count, caption: `${t.brands} brands` })), key, { title: "Territory ownership", hint: "pieces per territory" })];
    }
    case "whitespace": {
      // Plotting every territory produced an unreadable pile of overlapping labels. Keep
      // only territories with enough evidence to be real, then take the most telling ones:
      // the open+in-demand opportunities first, then the crowded leaders for contrast.
      ...
      // Cap per quadrant so the chart stays legible however many territories exist.
      const bucket = (o, w) => scored.filter((p) => p.open === o && p.wanted === w)
        .sort((a, b) => (o ? b.y - a.y : b.x - a.x)).slice(0, 4);
      const points = [...bucket(true, true), ...bucket(false, true), ...bucket(true, false), ...bucket(false, false)];
      return [quadrant({ points, xLabel: "Supply (brands publishing) →", yLabel: "↑ Demand", title: "White space" }, key)];
    }
    case "exec": {
      return [kpiRow([
