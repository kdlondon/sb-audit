...
const ENTRY_COLS =
  "id,competitor,brand,brand_name,scope,communication_intent,channel,year,rating," +
  "primary_territory,brand_archetype,tone_of_voice,main_slogan,synopsis,description,url,image_url,custom_dimensions,posted_at";
...
export async function resolveSource(supabase, projectId, { mode, value } = {}) {
  ...
  if (mode === "collection") {
    ...
    for (let i = 0; i < ids.length; i += 200) {
      const { data } = await supabase.from("creative_source").select(ENTRY_COLS).in("id", ids.slice(i, i + 200));
