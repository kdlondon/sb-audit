...
export const COMPETITOR_COLORS = {
  ...
  Desjardins:"#16a34a",Amex:"#0369a1",Venn:"#7c3aed",Float:"#14b8a6",
...
export function getFieldsForScope(scope) {
  return SHARED_SECTIONS.map(sec => ({
    ...sec,
    fields: sec.fields.filter(f => {
      if (scope === "local" && f.globalOnly) return false;
      if (scope === "global" && f.localOnly) return false;
      return true;
    })
  })).filter(sec => sec.fields.length > 0);
}

export function getTableName(scope) {
  return scope === "global" ? "audit_global" : "audit_entries";
}
