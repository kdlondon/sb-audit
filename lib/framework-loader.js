...
import { deriveFrameworkLists } from "@/lib/project-brands";
...
function createServerClient() {
  ...
  // Use service role key for server-side to bypass RLS
...
export async function loadFramework(projectId, supabaseClient) {
  ...
  // Prefer the normalized registry (project_brands) when it has competitor rows —
  // legacy arrays remain the fallback during the transition. See lib/project-brands.js.
  let brandLists = null;
  try {
    const { data: pb } = await supabase.from("project_brands").select("*").eq("project_id", projectId).eq("archived", false).order("sort_order");
    if (pb && pb.some((b) => b.role !== "principal")) brandLists = deriveFrameworkLists(pb);
  } catch {}
  ...
  return {
    ...
    // Framework data — support both column names
    dimensions: data.custom_dimensions || data.dimensions || [],
    customDimensions: data.custom_dimensions || data.dimensions || [],
    ...
    // Competitors/benchmarks — normalized registry first, legacy arrays as fallback
    localCompetitors: brandLists?.localCompetitors || data.local_competitors || [],
    globalBenchmarks: brandLists?.globalBenchmarks || data.global_benchmarks || [],
    projectBrands: brandLists?.projectBrands || [],
    principalBrand: brandLists?.principalBrand || null,
  };
}
  ...
  for (const dim of SYSTEM_DIMENSIONS) {
    for (const field of dim.fields || []) {
      ...
      if (field.type === "multichoice" && field.values?.length) {
        if (key === "communication_intent") {
          const intents = framework.communicationIntents?.length ? framework.communicationIntents : field.values;
          fields[key] = `One or more of: ${intents.join(" | ")} (comma separated). STRICT RULE — "Brand Hero" is RARE: reserve it ONLY for major brand-POSITIONING pieces — manifestos, brand films, flagship campaign commercials that define what the brand STANDS FOR (its purpose, identity, market stance). The vast majority of content is NOT Hero: promotions, offers, product/route pushes, seasonal/cultural/community posts, employee or behind-the-scenes, testimonials, reactive or tactical social → classify as Brand Tactical, Product, Client Testimonials, etc. If a piece is selling, promoting, informing, or filling the content calendar, it is TACTICAL, not Hero. When unsure, it is NOT Hero.`;
        } else {
