...
export function buildClassificationFields(framework) {
  ...
  for (const dim of SYSTEM_DIMENSIONS) {
    for (const field of dim.fields || []) {
      ...
      if (["toggle", "brand_selector", "country_search", "taxonomy", "url", "rating"].includes(field.type)) continue;
    ...
    if (dim.fields?.length) {
      ...
      for (const field of dim.fields) {
        if (field.type === "single_choice" && field.values?.length) {
          fields[field.key] = `MUST be: ${field.values.join(" | ")} | Other`;
        } else if (field.type === "multichoice" && field.values?.length) {
          fields[field.key] = `MUST be one or more of: ${field.values.join(" | ")} | Other — comma separated if multiple`;
        } else {
          fields[field.key] = `${field.name} — brief description or classification`;
        }
      }
    ...
    if (dim.fields?.length) {
      ...
      for (const field of dim.fields) {
        if (field.values?.length) {
          context += `- ${field.name}: ${field.values.join(", ")}\n`;
        }
      }
