  ...
  const loadFramework = useCallback(async (bid) => {
    ...
    try {
      ...
      try {
        ...
        if (pid) {
          const { data: pb } = await supabase.from("project_brands").select("*").eq("project_id", pid).eq("archived", false).order("sort_order");
      ...
      setFramework({
        ...
        // Support both column names: dimensions (project_frameworks) and custom_dimensions (brand_frameworks)
