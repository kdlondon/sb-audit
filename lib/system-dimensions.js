...
export const SYSTEM_DIMENSIONS = [
  {
    ...
    fields: [
      ...
      { key: "category_proximity", name: "Category proximity", type: "multichoice", values: ["Direct", "Adjacent", "Parallel", "Distant"] },
...
export function getAllDimensions(brandFramework) {
  ...
  const custom = (brandFramework?.custom_dimensions || []).map((d, i) => ({
    ...d,
    is_system: false,
    sort_order: d.sort_order ?? (100 + i),
  }));
  return [...system, ...custom].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
}
