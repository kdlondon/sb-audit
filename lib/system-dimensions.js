...
export const SYSTEM_DIMENSIONS = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "execution_style", name: "Execution style", type: "multichoice", values: ["Live action", "Animation", "Mixed media", "Typography", "Stock footage", "UGC", "Illustration", "Cinematic", "Documentary", "Manifesto", "Motion graphics", "Photography", "Data visualisation", "Interactive", "AR/VR", "Other"] },
...
export function getAllDimensions(brandFramework) {
  ...
  const custom = customRaw.map((d, i) => ({
    ...
    // A custom dimension may be stored in the OLDER single-field shape — { key, name,
    // values } — where the dimension IS one field. Settings adapts that on the fly, but
    // the audit form iterates `dim.fields` and so silently dropped every such dimension
    // (all 12 of Scotiabank's). Normalise here, once, so every consumer sees `fields`.
    fields: (Array.isArray(d.fields) && d.fields.length)
      ? d.fields
      : [{ key: d.key, db_key: d.key, name: d.name, type: d.type || "single_choice", values: d.values || [], description: d.description }],
  }));
...
export function getAllFieldKeys(brandFramework) {
  ...
  const keys = new Set(["id", "organization_id", "brand_id", "project_id", "scope", "brand_name",
    ...
    // Origin axes: where a piece came from + how it was acquired + its native id.
    "source_platform", "source_type", "source_ref"]);
  for (const dim of dims) {
    const fields = Array.isArray(dim.fields) ? dim.fields : [];
    for (const field of fields) {
      if (!field || typeof field !== "object") continue;
      const k = field.db_key || field.key;
      // Skip invalid keys (single chars, empty strings, non-strings)
      if (typeof k === "string" && k.length > 1) keys.add(k);
    }
  }
  return [...keys];
}
