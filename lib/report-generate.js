...
export async function generateReport({ card, config, postSection, saveDoc, onProgress } = {}) {
  ...
  const persist = () => {
    if (!saveDoc) return;
    saveChain = saveChain.then(() => saveDoc(assembleDoc(card, config, produced), produced.map((s) => s.key))).catch(() => {});
    return saveChain;
  };
  ...
  const TRANSIENT = /429|rate.?limit|overloaded|529|503|502|504|timeout|ECONN/i;
  ...
  // PASS 1 — analytical sections in PARALLEL. They are independent, and the engines were
  // built to run them concurrently; doing them one at a time turned a ~50s report into
  // several minutes.
  await Promise.all(analytical.map((key) => runOne(key, [])));

  // PASS 2 — synthesis sections, which need the analytical prose as context.
  if (synthesis.length) {
    const prior = produced.filter((s) => !SYNTHESIS.has(s.key));
    await Promise.all(synthesis.map((key) => runOne(key, prior)));
  }

  await saveChain;
  return { doc: assembleDoc(card, config, produced), produced, failed, errors, complete: failed.length === 0 };
}
