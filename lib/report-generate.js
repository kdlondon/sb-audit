...
export function sectionToBlocks(section) {
  ...
  // The eyebrow is the fixed section name, the lead the engine's one-line setup for THIS
  // report. Together with the numeral they make every section open the same way.
  if (section.lead || section.title) {
    blocks.push(makeBlock("lead", { sectionKey: section.key, data: { text: section.lead || "", eyebrow: section.title || "" } }));
  }
