import { performanceScore } from "./engagement";
...
export function pieceWeight(piece = {}, { section, mode = "brand_signal", refYear } = {}) {
  if (mode === "performance") {
    // Engagement RATE (interactions/followers) is the driver — same helper the whole app
    // uses — times recency. Legacy no-follower content falls back inside performanceScore.
    return performanceScore(piece) * recencyFactor(piece, refYear);
  }
  if (mode === "quality") {
